...
class AIODockerHelper:
    ...
    async def setup_host_image(self, remote_path=None, tag=None):
        ...
        try:
            docker_client = await self.get_docker_client()
            if remote_path and tag is not None:
                params = {"tag": tag, "remote": remote_path}
                await docker_client.images.build(**params)

            else:
                image = await docker_client.images.list(filter=self.host_image)
                if not image:
                    await docker_client.images.pull(self.host_image)

        except aiodocker.exceptions.DockerError as docker_error:
            self.logger.exception(
                "Error while pulling %s image %s", self.host_image, docker_error
            )
    ...
    async def get_container(self, container_name):
        ...
        try:
            docker_client = await self.get_docker_client()
            container = await docker_client.containers.get(container=container_name)

        except aiodocker.exceptions.DockerError as server_error:
            self.logger.exception(
                "Error while fetching %s container %s", container_name, server_error
            )
        return container
    ...
    async def create_container(self, container_name, cmd=None, image=None):
        ...
        try:
            ...
            or aiodocker.exceptions.DockerContainerError
        ) as docker_error:
    ...
    async def execute_cmd(self, cmd, image=None):
        ...
        try:
            ...
            self.logger.error(
                "Error while executing command %s in container %s", cmd, server_error
            )
    ...
    async def delete_container(self, container_name):
        ...
        try:
            ...
            self.logger.exception(
                "Error while removing %s container %s", container_name, server_error
            )
